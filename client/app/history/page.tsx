  ...
  if (!userId) {
    return (
      <main className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-white p-8">
        <p className="text-gray-500 dark:text-gray-400">Log in to view your history.</p>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-white p-8">
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className={`px-4 py-2 rounded ${tab === "orders" ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-800"}`}
        >
          Orders
        </button>
        <button
          onClick={() => setTab("trades")}
          className={`px-4 py-2 rounded ${tab === "trades" ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-800"}`}
        >
          Trades
        </button>
      </div>
      ...
      {tab === "orders" && (
        <>
          {ordersLoading && <p className="text-gray-500 dark:text-gray-400">Loading...</p>}
          {!ordersLoading && orders.length === 0 && <p className="text-gray-500 dark:text-gray-400">No orders yet.</p>}
          {orders.length > 0 && (
            <table className="w-full text-left">
              ...
              <tbody>
                {orders.map((o) => (
                  <tr key={o.id} className="border-b border-gray-200 dark:border-gray-800">
                    <td className="py-2">{o.symbol}</td>
                    <td className={`py-2 ${o.side === "Buy" ? "text-green-600 dark:text-green-500" : "text-red-600 dark:text-red-500"}`}>{o.side}</td>
      ...
      {tab === "trades" && (
        <>
          {tradesLoading && <p className="text-gray-500 dark:text-gray-400">Loading...</p>}
          {!tradesLoading && trades.length === 0 && <p className="text-gray-500 dark:text-gray-400">No trades yet.</p>}
          {trades.length > 0 && (
            <table className="w-full text-left">
              <thead className="text-gray-500 dark:text-gray-400 border-b border-gray-300 dark:border-gray-700">
                <tr>

...
export function AlertList({ alerts }: { alerts: AlertRecord[] }) {
  if (alerts.length === 0) {
    return <p className="text-gray-500 dark:text-gray-400">No alerts set.</p>;
  }
      ...
      <tbody>
        {alerts.map((a) => (
          <tr key={a.id} className="border-b border-gray-200 dark:border-gray-800">
            ...
            <td className="py-2">
              {a.triggered ? (
                <span className="px-2 py-1 rounded text-xs bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300">Triggered</span>
              ) : (
                <span className="px-2 py-1 rounded text-xs bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300">Active</span>
              )}
            </td>
          </tr>
        ))}
